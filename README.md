...
Phase 44では、カレンダーでの計測なしの記録追加と、編集画面で確認後に行う保存済み記録の完全削除をローカル実装します。過去・未来の時間帯と既存記録との重複を許可します。追加機能の本番D1 migration・Worker deployは別承認のPhase 45で行うため、現在の本番にはまだ反映していません。
